...
void handleRoot() {
  ...
  <p id="irDisplay">N/A</p>
      ...
      <button onclick="fetch('/motor2FWD')">Inainte 180</button>
      ...
      <button onclick="fetch('/servo3_75')">Pozitie 60°</button>
...
// void handleSet() { // Aceasta este o versiune mai simplă a funcției handleSet care nu include logica de boost
...
void handleSet() {
  static unsigned long lastBoost = 0;
  ...
  if (server.hasArg("val")) {
    ...
    // Aplică valoarea cerută
    myServo.write(val);

    // Dacă a trecut 1 secundă de la ultimul boost:
    if (millis() - lastBoost >= 1000 && val != 90) {
      int boostVal = val;
      if (val < 90) boostVal = constrain(val - 5, 0, 180);
      else if (val > 90) boostVal = constrain(val + 5, 0, 180);

      delay(20);  // scurt delay ca să se aplice valoarea inițială
      myServo.write(boostVal);
      delay(20);  // boost scurt
      myServo.write(val);  // revine la valoarea cerută

      lastBoost = millis();  // actualizează timerul boostului
    }
  }
